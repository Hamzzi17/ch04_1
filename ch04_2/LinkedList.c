...
linkedList_h* createLinkedList_h(void) {
	...
	L->head = NULL;	// 공백 리스트이므로 NULL로 결정
	return L;
}
...
void insertFirstNode(linkedList_h* L, char* x) {
	listNode* newNode;
	newNode = (listNode*)malloc(sizeof(listNode));	// 삽입할 새 노드 생성
...
void insertMiddleNode(linkedList_h* L, listNode* pre, char* x) {
	...
	if (L->head == NULL) {				// 공백 리스트인 경우,
		newNode->link = L->head;		// 새 노드를 첫 번째이자 마지막 노드로 연결
		L->head = newNode;
	}
...
void insertLastNode(linkedList_h* L, char* x) {
	...
	// 현재 리스트가 공백이 아닌 경우
	temp = L->head;
	while (temp->link != NULL)temp = temp->link;	// 현재 라스트의 마지막 노드를 찾음
...
void  deleteNode(linkedList_h* L, listNode* p) {
	...
	else {							// 삭제할 노드 p의 선행자 노드를 포인터pre를 이용해 찾음
		pre = L->head;
		while (pre->link != p) {
			pre = pre->link;
		}
